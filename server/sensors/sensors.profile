 ...
 */
function sensors_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function sensors_install_tasks() {
  $tasks = array();

  $tasks['sensors_setup_variables'] = array(
    'display_name' => st('Set Variables'),
    'display' => FALSE,
  );

  // Run this as the last task!
  $tasks['sensors_setup_rebuild_permissions'] = array(
 ...
 */
function sensors_setup_variables() {
  $variables = array(
    // Features default export path.
    'features_default_export_path' => 'profiles/sensors/modules/custom',
    // Mime-mail.
    'mimemail_format' => 'full_html',
    'mimemail_sitestyle' => FALSE,
    'mimemail_name' => 'Sensors',
    'mimemail_mail' => 'info@sensors.com',
    // jQuery versions.
 ...
 * installation.
 */
function sensors_setup_rebuild_permissions() {
  node_access_rebuild();
}
