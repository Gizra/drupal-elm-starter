...
class HedleyRestfulPusherAuthResource extends RestfulEntityBase {
  ...
  public function authenticatePusher() {
    ...
    if (!user_access('access private fields', $account)) {
      throw new \RestfulForbiddenException(format_string('User @name is trying to log into the privileged pusher channel "@channel", but has no "access private fields" permission.', ['@name' => $account->name, '@channel' => $request['channel_name']]));
    }
