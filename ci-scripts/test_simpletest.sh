...
cd "$ROOT_DIR"/server
php ./www/scripts/run-tests.sh --cache --cache-modules --php "$(which php)" --concurrency 4 --verbose --color --url http://127.0.0.1:8080 Hedley 2>&1 | tee /tmp/simpletest-result.txt
