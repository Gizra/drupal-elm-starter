...
date
php ./www/scripts/run-tests.sh --cache --cache-modules --php "$(which php)" --concurrency 4 --verbose --color --url http://server.local SimpleTest 2>&1 | tee /tmp/simpletest-result.txt
