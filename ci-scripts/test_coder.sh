...
code_review () {
  ...
  if [[ "${LOCAL_MODE}" -eq 0 ]]; then
    if ! "$CMD" --standard="$REVIEW_STANDARD" -p --colors --extensions=php,module,inc,install,test,profile,theme,js,css "$1"; then
      HAS_ERRORS=1
    fi
  else
    if ! ddev exec "$CMD" --standard="$REVIEW_STANDARD" -p --colors --ignore=profiles/contrib/*  --extensions=php,module,inc,install,test,profile,theme,js,css "${1#web/}"; then
      HAS_ERRORS=1
    fi
  fi
}
